...
import {
  HISTORY_WITHDRAW_API,
  HISTORY_WITHDRAW_API_M3,
} from "./../../../const/const";
...
const columns = [
  {
    ...
    dataIndex: "tx",
    key: "hash",
    render: (e) => (e ? <span>{formatAddress(e)}</span> : "Pending"),
  },
  {
    title: "Amount",
    dataIndex: "amount",
...
const App = (props) => {
  ...
  const handleChangePagination = async (e) => {
    await fetchData(HISTORY_WITHDRAW_API, e - 1, 5, 0);
  };

  const handleChangePagination_m3 = async (e) => {
    await fetchData_m3(HISTORY_WITHDRAW_API_M3, e - 1, 5, 0);
  };

  const handleChangePagination_m3_BUSD = async (e) => {
    await fetchData_m3_BUSD(HISTORY_WITHDRAW_API_M3, e - 1, 5, 0);
  };

  useEffect(() => {
    if (mode === MODES[0]) {
      if (props.isLoadPage1 || _meta.address) {
        fetchData(HISTORY_WITHDRAW_API, 0, 5, 1);
      } else {
        setDataTable([]);
        setTotalRecord(0);
      }
    }
  }, [props.isLoadPage1, _meta.address]);

  useEffect(() => {
    if (mode === MODES[1]) {
      if (props.isLoadPage3 || _meta.address) {
        fetchData_m3(HISTORY_WITHDRAW_API_M3, 0, 5, 1);
      } else {
        setDataTable_m3([]);
        setTotalRecord_m3(0);
      }
    }
  }, [props.isLoadPage3, _meta.address]);

  useEffect(() => {
    if (mode === MODES[2]) {
      if (props.isLoadPage5 || _meta.address) {
        fetchData_m3_BUSD(HISTORY_WITHDRAW_API_M3, 0, 5, 1);
      } else {
        setDataTable_m3_BUSD([]);
        setTotalRecord_m3_BUSD(0);
      }
    }
  }, [props.isLoadPage5, _meta.address]);
