...
const TheBai = ({ onComplete, animation, skins = [] }) => {
  ...
  useEffect(() => {
    ...
    const shared = app.loader;
    skins.forEach((skin) => {
      if (!checkSkinAvailable(skin)) {
        shared.add([
          {
            name: `${skin}`,
            url: `https://storage.googleapis.com/fbnc-finance/fifa/all/${skin}.png`,
            crossOrigin: false,
          },
        ]);
      }
    });
    function loadProgressHandler(loader, _) {
      if (loader.progress < 100) setLoading(true);
      else setLoading(false);
    }
    //Use Pixi's built-in `Loader` module to load an image
    shared.onProgress.add(loadProgressHandler);

    shared.add("thebai", "images/TheBai/thebai.json").load((_, res) => {
